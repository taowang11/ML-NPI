...
class ML_NPI(torch.nn.Module):
    ...
    def global_update(self, support_x, support_y, query_x, query_y):
        ...
        for i in range(task_size_s):
            ...
            if self.model_name == 'time_interval_sp':
                _loss_s, _acc_s, _ap_s, _f1_s, _auc_s,_aupr,_precision,_recal,_specificity \
        ...
        return loss.cpu().detach().numpy(), acc_s, ap_s, f1_s, auc_s,aupr,precision,recall,specificity
    ...
    def evaluate(self, support_x, support_y, query_x, query_y):
        ...
        return acc, ap_s, f1, auc_s,aupr,precision,recall,specificity
    ...
    def time_MAML(self, support_x, support_y, query_x, query_y):
        ...
        acc, ap, f1, auc,aupr,precision,recal,specificity= self.cal_metrics.prediction(np.array(query_y).astype(int),
                                                       pred_sore.cpu().detach().numpy())

        return loss, acc, ap, f1, auc,aupr,precision,recal,specificity
