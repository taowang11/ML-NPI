...
class TGNN_Encoder(torch.nn.Module):
    def __init__(self, config, ngh_finder, n_feat, e_feat):
        ...
        self.n_feat_dim = config['embedding_dim']
        self.e_feat_dim = config['embedding_dim']
        self.time_dim = config['time_embedding_dim']
        ...
        w1 = torch.nn.Parameter(torch.ones(2*self.n_feat_dim, self.model_dim + self.n_feat_dim))
        torch.nn.init.xavier_normal_(w1)
        self.vars['agg_fc_w1'] = w1

        w2 = torch.nn.Parameter(torch.ones(self.n_feat_dim, 2*self.n_feat_dim))
        torch.nn.init.xavier_normal_(w2)
